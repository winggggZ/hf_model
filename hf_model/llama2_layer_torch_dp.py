...
# data  batch
inputs = torch.rand(args.batch, config.seq_len, config.hidden_size).to(device)
...
# model
model = Llama2_already_emb(config=config).to(device)
...
print(f'batch_size: {args.batch} llama done')
